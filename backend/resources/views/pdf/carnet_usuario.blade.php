    ...
    <div class="carnet">
        ...
        <div class="photo-area">
            <div class="photo-frame">
                @php
                    $foto = null;
                    if ($usuario->persona && $usuario->persona->foto) {
                        if (filter_var($usuario->persona->foto, FILTER_VALIDATE_URL)) {
                            $foto = $usuario->persona->foto;
                        } else {
                            // Usar ruta directa real de storage/app/public para que domPDF la lea sin importar symlinks
                            $rawPath = ltrim($usuario->persona->foto, '/');
                ...
                @else
                    <div class="no-photo">
                        <div class="no-photo-text">{{ strtoupper(substr($usuario->persona->nombres ?? 'U', 0, 1)) }}</div>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="info-area">
            <div class="label-top">Operador Autorizado</div>
            <div class="name-big" style="font-size: {{ $fontSize }};">
                {{ strtoupper($usuario->persona->nombres ?? '') }}<br>
                {{ strtoupper($usuario->persona->apellidos ?? '') }}
            </div>

            <div class="info-item">
                <div class="info-label">Cédula Identidad</div>
                <div class="info-val">{{ $usuario->cedula }}</div>
            </div>

            <div class="info-item">
                <div class="info-label">Rol en el Sistema</div>
                <div class="info-val yellow">{{ strtoupper($usuario->rol ?? 'USUARIO') }}</div>
            </div>

            <div class="info-item">
                <div class="info-label">Email Institucional</div>
                <div class="info-val" style="color: #cbd5e1; font-size: 1.5mm; line-height: 1.15;">{{ strtolower($usuario->email) }}</div>
            </div>

            <div class="info-item">
                <div class="info-label">Estado de Acceso</div>
                <div class="info-val" style="color: {{ $usuario->estado ? '#10b981' : '#ef4444' }};">{{ $usuario->estado ? 'ACTIVO' : 'RESTRINGIDO' }}</div>
            </div>

            @if(isset($usuario->persona->carrera) && $usuario->persona->carrera)
            <div class="info-item">
                <div class="info-label">Cargo / Carrera</div>
                <div class="info-val" style="line-height: 1.15;">{{ strtoupper($usuario->persona->carrera) }}</div>
            </div>
            @endif

            @if(isset($usuario->persona->facultad) && $usuario->persona->facultad)
            <div class="info-item">
                <div class="info-label">Unidad Administrada</div>
                <div class="info-val" style="line-height: 1.15;">{{ strtoupper($usuario->persona->facultad) }}</div>
            </div>
            @endif
        </div>
        ...
        <div class="dorsal-section">
            <div class="dorsal-label">Nivel</div>
            <div class="dorsal-num" style="font-size: 3.5mm;">{{ strtoupper(substr($usuario->rol ?? 'USR', 0, 3)) }}</div>
        </div>
        ...
        <div class="footer-line">
            VALIDACIÓN: {{ strtoupper(substr(md5($usuario->cedula ?? 'X'), 0, 10)) }} | EMISIÓN: {{ date('d/m/Y') }}
        </div>
    </div>
</body>
</html>