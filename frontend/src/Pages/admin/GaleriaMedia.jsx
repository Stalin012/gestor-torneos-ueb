...
const GaleriaMedia = () => {
    ...
    const handleSave = async (form, isEdit, id) => {
        ...
        try {
            ...
            let resp;
            if (isEdit) {
                // En Laravel, para enviar archivos con PUT se usa POST + _method=PUT
                fd.append("_method", "PUT");
                resp = await api.post(`/galeria/${id}`, fd, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            } else {
                resp = await api.post('/galeria', fd, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            }
    ...
    const handleDelete = async (id) => {
        ...
        setLoading(true);
        try {
            const resp = await api.delete(`/galeria/${id}`);
            if (resp.status === 200) {
                alert(resp.data.message || "Eliminado correctamente");
                loadData();
            }
        } catch (e) {
            console.error(e);
    ...
    if (loading && list.length === 0) return <LoadingScreen message="Abriendo Galería..." />;
